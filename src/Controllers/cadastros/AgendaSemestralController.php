...
class AgendaSemestralController
{
    ...
    public function index(
        ...
        return $this->twig->render(
            $response,
            'cadastros/semestres/index.html.twig',
            $this->ctx(['semestres' => $semestres])
        );
    }
    ...
    public function create(
        ...
        return $this->twig->render(
            $response,
            'cadastros/semestres/form.html.twig',
            $this->ctx([
                'semestre' => null,
                'modo'     => 'criar',
            ])
    ...
    public function store(
        ...
        // Valida unicidade da referência
        ...
        $_SESSION['flash_success'] = 'Semestre criado com sucesso. As ' .
            ($data['num_semanas'] ?? 20) . ' semanas foram geradas automaticamente.';

        return $response->withHeader(
            'Location',
            "/cadastros/semestres/{$id}"
        )->withStatus(302);
    }

    // -------------------------------------------------------------------------
    // Detalhe (semanas + dias bloqueados + form de novo dia bloqueado)
    // -------------------------------------------------------------------------
    ...
    public function show(
        ...
        $id       = (int) $args['id'];
        $semestre = $this->repo->findById($id);
        if (!$semestre) {
            return $response->withStatus(404);
        }

        $semanas        = $this->repo->getSemanas($id);
        $diasBloqueados = $this->repo->getDiasBloqueados($id);

        // Dados do formulário POST (re-exibição em caso de erro)
        ...
        return $this->twig->render(
            $response,
            'cadastros/semestres/show.html.twig',
            $this->ctx([
                'semestre'        => $semestre,
                'semanas'         => $semanas,
    ...
    // -------------------------------------------------------------------------
    // Ações adicionais (também mapeadas em routes.php se necessário)
    // -------------------------------------------------------------------------

    /**
     * Adiciona um dia bloqueado ao semestre via POST.
     * Redireciona de volta ao show.
     */
    public function addDiaBloqueado(
        ...
        $this->repo->addDiaBloqueado($id, $data);
    ...
    /**
     * Remove um dia bloqueado e redireciona ao semestre.
     */
    public function removeDiaBloqueado(
        ...
        $semestreId = (int) $args['id'];
        $diaId      = (int) $args['dia_id'];

        $this->repo->removeDiaBloqueado($diaId);

        $_SESSION['flash_success'] = 'Dia bloqueado removido com sucesso.';
        return $response->withHeader(
            'Location',
            "/cadastros/semestres/{$semestreId}"
        )->withStatus(302);
    }

    /**
     * Ativa o semestre informado.
     */
    ...
    private function validarDiaBloqueado(array $data, array $semestre): array
    {
        ...
        if (empty($data['data'])) {
            ...
            // Verifica se a data cai dentro do semestre
            if (
                $data['data'] < $semestre['data_inicio']
                || $data['data'] > $semestre['data_fim']
            ) {
                $errors['data'] = 'A data deve estar dentro do período do semestre ('
                    . $semestre['data_inicio'] . ' a ' . $semestre['data_fim'] . ').';
            }
        }
        ...
        $tiposValidos = ['feriado', 'recesso', 'evento', 'manutencao'];
