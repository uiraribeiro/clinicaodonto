...
class LaboratorioRepository
{
    ...
    public function findById(int $id): ?array
    {
        $stmt = $this->pdo->prepare(
            'SELECT * FROM laboratorios WHERE id = :id LIMIT 1'
        );
        $stmt->execute([':id' => $id]);
        $row = $stmt->fetch();
        return $row ?: null;
    ...
    public function update(int $id, array $data, int $usuarioId): void
    {
        ...
        $stmt->execute([
            ':id'                  => $id,
            ':nome'                => trim($data['nome']),
            ':quantidade_assentos' => (int) $data['quantidade_assentos'],
    ...
    /**
     * Retorna bloqueios futuros (data_fim >= hoje) para o laboratório informado.
     */
    public function getBloqueios(int $laboratorioId): array
    {
        $stmt = $this->pdo->prepare('
            SELECT be.*, u.nome AS criado_por_nome
            FROM bloqueios_espaco be
            LEFT JOIN usuarios u ON u.id = be.created_by
            WHERE be.espaco_tipo = \'laboratorio\'
              AND be.espaco_id   = :laboratorio_id
              AND be.data_fim   >= CURDATE()
            ORDER BY be.data_inicio, be.hora_inicio
        ');
        $stmt->execute([':laboratorio_id' => $laboratorioId]);
        return $stmt->fetchAll();
    }

    public function addBloqueio(int $laboratorioId, array $data, int $usuarioId): int
    {
        $stmt = $this->pdo->prepare('
            ...
            VALUES
                (\'laboratorio\', :espaco_id, :data_inicio, :data_fim,
                 :hora_inicio, :hora_fim, :motivo, :created_by)
        ');

        $stmt->execute([
            ':espaco_id'   => $laboratorioId,
    ...
    {
        $this->pdo->prepare(
            "DELETE FROM bloqueios_espaco WHERE id = :id AND espaco_tipo = 'laboratorio'"
        )->execute([':id' => $bloqueioId]);
    }

    /**
     * Verifica se o laboratório está disponível em uma data e intervalo de horas.
    ...
    public function estaDisponivelNaData(
        ...
        $stmt = $this->pdo->prepare('
            ...
            WHERE espaco_tipo  = \'laboratorio\'
              AND espaco_id    = :laboratorio_id
